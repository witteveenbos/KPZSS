...
for scene, file in scene_dict.items():
    ...
    for hubname in hubnames:
        ...
        df_xy_ahn['distance'] = np.sqrt((df_xy_ahn['x']-df_xy_ahn['x'][0])**2+(df_xy_ahn['y']-df_xy_ahn['y'][0])**2)        
        ...
        df_xy_ahn   = df_xy_ahn.drop(df_xy_ahn[df_xy_ahn['distance'] > max_len].index)
        ...
        # interpolate bathy to profiles
        interpolate.interpolate_xyz_on_xy(df_xyz, df_xy)
        interpolate.interpolate_xyz_on_xy(df_xyz_ref, df_xy_ref)
        interpolate.interpolate_xyz_on_xy(df_xyz_ahn, df_xy_ahn)
        interpolate.interpolate_xyz_on_xy(df_xyz_ahn, df_xy_ahn_ref)
        interpolate.interpolate_xyz_on_xy(df_xyz, df_xy_comb)
           
        df_xy[df_xy['z']<-100]['z'] = np.nan
        df_xy_ref[df_xy_ref['z']<-100]['z'] = np.nan
        
        df_dh['z_ref'] = df_xy_ref['z']
        df_dh['z_scen'] = df_xy['z']
        df_dh['dh'] = df_dh['z_scen'] - df_dh['z_ref'] 
        df_dh[df_dh['dh']>10] = np.nan
        df_dh[df_dh['dh']<-10] = np.nan 
               
        # fill out nans
        df_dh['dhfilled'] = df_dh['dh'].interpolate()
        df_dh[df_dh['dhfilled'].isna()] = 0
        
        # add dh to ahn data
        df_xy_ahn['z'] = df_xy_ahn['z'] +  df_dh['dhfilled']
        
        # if ahn-data is available, replace values from deltares bathy with the values from ahn
        df_xy_comb.loc[df_xy_ahn['z'].notnull().values] = df_xy_ahn.loc[df_xy_ahn['z'].notnull().values]
        
        # make buffer for interpolation on edge bathy and ahn if enough ahn datapoints
        buff_data_outside = df_xy_ahn['z'][::-1].notnull().idxmax() + 1
        buffer_steps = buffer_ahn_deltares / spacing
        if buff_data_outside > 0:
            df_xy_comb.loc[buff_data_outside:(buff_data_outside+buffer_steps), 'z'] = np.nan
        else:
        ...
        ax[0].set_xlim([df_xy['x'][0] - 610, df_xy['x'][0] + 610])
        ax[0].set_ylim([df_xy['y'][0] - 610, df_xy['y'][0] + 610])
        ...
        ax[1].plot(df_xy['distance'],df_xy['z'],'-',color='c',label='Deltares scenario',linewidth = 4)
        
        # ahn bathy
        ax[1].plot(df_xy_ahn_ref['distance'],df_xy_ahn_ref['z'],'-',color='tab:pink',label='AHN referentie',linewidth = 2)
        ax[1].plot(df_xy_ahn['distance'],df_xy_ahn['z'],'-',color='tab:pink',label='AHN scenario',linewidth = 4)
        ...
        ax[1].set_title(f'Bodemhoogte langs profiel, dx = {spacing} m, DN = {FC_DN}')
